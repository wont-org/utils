 ...
 * @param {array} tree - 格式化前的tree
 * @param {object} [fieldNames={id: 'key', label: 'label', children: 'children'}] - 格式化tree所需的属性
 * @param {boolean} [parentSelectable = false] - 父级是否可选择，默认不可选
...
formatTreeSelectProps(beforeTree)  // return afterTree
 */
...
export const formatTreeSelectProps: FormatTreeSelectProps = ({
    ...
    if (Array.isArray(item[fieldNames.children]) && item[fieldNames.children].length > 0) {
        return {
            ...item,
